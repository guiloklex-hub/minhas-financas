...
- **Import de fatura por IA (PDF/imagem):** [ai-invoice-import.ts](src/actions/ai-invoice-import.ts) — `extractInvoiceForImport` (fase A, não grava) lê o PDF/imagem via Gemini multimodal (`inlineData`, magic bytes), transcreve **só os lançamentos reais** (exclui "próximas faturas"/totais/limites), normaliza em [invoice-import.ts](src/lib/invoice-import.ts) (`sanitizeInvoiceLine`/`parseInstallment`/`dedupKeyCard`/`sourceKey`, puros e testados), deduplica contra o cartão e sugere categoria (histórico + IA). `confirmInvoiceImport` (fase B) grava como `CreditCardTransaction` no físico, atribuindo `virtualCardId` pelo mapeamento de origens (`@`/"final"), por competência (`getInvoiceCompetence` + `ensureInvoice`), sem confiar no client (valida categoria, deduplica, atômico). Faturas Itaú são **imagem** (sem texto) → parser determinístico inviável; só IA multimodal. Requer modelo Gemini com suporte a PDF.
- **Cartões virtuais (filhos do físico):** model `VirtualCard` (vinculado a um `CreditCard`, com `spendingLimit` opcional). A compra continua com `cardId` = **físico** (cai na mesma fatura/competência) e recebe só a marcação `virtualCardId` — validada em [virtual-cards.ts](src/actions/virtual-cards.ts) e em `createCardPurchase`/`createCardRefund` (`virtualCard.cardId === cardId`). Por isso **fatura, limite, recompensas, relatórios e IA não precisam mudar** (já agregam por `cardId` físico). `computeVirtualCardUsage` (puro) calcula o uso por virtual (fatura atual + futuras) para a barra de sub-limite. A fatura exibe selo por linha + filtro/total por cartão.
...
- **Models:** `User`, `Account`, `Category`, `Budget`, `Transaction` (`isTransfer`, `transferGroupId`, `recurrenceGroupId`, `notes`, `tags`, `reconciled`, `creditCardInvoiceId`), `Investment`, `AiUsageLog`, `RecurringRule`, `Goal`, `Notification`, `PushSubscription`, `AuditLog`, `MonthlyInsight`, `ExchangeRate`, `CreditCard`, `CreditCardTransaction`, `CreditCardInvoice`, `CardRewardLedger`, `VirtualCard`.
