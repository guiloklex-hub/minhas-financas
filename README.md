...
- **Import de CSV com IA** — ao importar um extrato (`Data,Título,Valor`), o sistema categoriza cada lançamento de forma híbrida (histórico grátis + IA só no restante) e mostra uma **pré-visualização editável** antes de confirmar; linhas sem sugestão caem numa categoria padrão escolhida na tela. O parser aceita separador `,` ou `;`, valor em formato BR (`1.234,56`) e data `DD/MM`, `DD/MM/AAAA` ou `AAAA-MM-DD`.
- **Import de CSV no cartão** — importa lançamentos do cartão por CSV (`Data,Descrição,Valor` + colunas opcionais `Tipo`, `Parcela` e `Cartão`), lido localmente (sem IA na leitura). Tem a mesma pré-visualização editável, dedup por lançamento, categorização híbrida (histórico + IA) e mapeamento de cartões virtuais (coluna `final XXXX`/`@XXXX`) do import por IA. Os lançamentos não creditam pontos de recompensa (igual ao import por IA).
