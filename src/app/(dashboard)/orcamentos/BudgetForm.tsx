    ...
    <form id="budget-form" action={handleSubmit} className="space-y-4 p-6 rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="md:col-span-2">
          ...
          <select required id="categoryId" name="categoryId" className="w-full bg-background border border-[var(--color-border)] rounded-md p-2 focus:outline-none focus:border-white/50 transition-colors">
        ...
        <div>
          ...
          <input required type="number" step="0.01" min="0" id="amountLimit" name="amountLimit" className="w-full bg-background border border-[var(--color-border)] rounded-md p-2 focus:outline-none focus:border-white/50 transition-colors" placeholder="0.00" />
        </div>
        <div className="grid grid-cols-2 gap-2">
          <div>
            <label htmlFor="month" className="block text-sm font-medium mb-1">Mês</label>
            <input required type="number" min="1" max="12" id="month" name="month" defaultValue={currentMonth} className="w-full bg-background border border-[var(--color-border)] rounded-md p-2 focus:outline-none focus:border-white/50 transition-colors" />
          </div>
          <div>
            <label htmlFor="year" className="block text-sm font-medium mb-1">Ano</label>
            <input required type="number" min="2000" id="year" name="year" defaultValue={currentYear} className="w-full bg-background border border-[var(--color-border)] rounded-md p-2 focus:outline-none focus:border-white/50 transition-colors" />
          </div>
        </div>
      </div>
